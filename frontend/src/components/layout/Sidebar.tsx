...
export default function Sidebar() {
  ...
  // Verifica se precisa forçar a troca de senha
  useEffect(() => {
  ...
  const handlePasswordChangeSuccess = () => {
    ...
    setForcePasswordChange(false);
    // Idealmente recarregar o user aqui para atualizar o must_change_password localmente
    // Como o login faz o fetch, um reload de pagina ou chamar uma funcao refreshUser resolveria
    ...
    <>
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col h-screen fixed left-0 top-0 z-10">
        ...
        <div className="p-4 border-t border-slate-100">
          ...
          >
            <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 font-semibold text-sm uppercase group-hover:bg-primary group-hover:text-white transition-colors">
      ...
      {/* Modal de Alterar Senha (Manual ou Forçado) */}
      {(isPasswordModalOpen || forcePasswordChange) && (
        <ChangePasswordModal 
