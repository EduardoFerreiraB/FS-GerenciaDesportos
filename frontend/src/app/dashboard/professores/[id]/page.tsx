...
export default function EditarProfessorPage() {
  ...
  const { data: professor, error, isLoading } = useSWR(id ? `/professores/${id}` : null, fetcher);
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] text-red-500">
        ...
        <p className="text-slate-600 mb-6">Não foi possível encontrar os dados deste professor.</p>
        <button 
  ...
  return <ProfessorForm initialData={professor} isEditing={true} />;
}