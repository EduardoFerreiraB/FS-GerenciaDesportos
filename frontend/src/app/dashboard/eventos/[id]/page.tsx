...
export default function DetalhesEventoPage() {
  ...
  const [edicaoModalState, setEdicaoModalState] = useState<{ isOpen: boolean; mode: 'create' | 'edit'; data?: any }>({ isOpen: false, mode: 'create' });
  ...
  // Estado da edição ativa dentro do evento
  ...
  const { data: todasEdicoes, mutate: mutateEdicoes, isLoading: isLoadingEdicoes } = useSWR('/edicoes/', fetcher);
  ...
  // Definir edição ativa padrão (a mais recente)
  useEffect(() => {
  ...
  // Edição ativa selecionada
  const activeEdicao = useMemo(() => {
    return edicoes.find((e: any) => e.id_edicao === activeEdicaoId) || null;
  }, [edicoes, activeEdicaoId]);

  // Fetch equipes e partidas da edição ativa
  const { data: equipes, mutate: mutateEquipes, isLoading: loadingEquipes } = useSWR(
    activeEdicaoId ? `/equipes/edicao/${activeEdicaoId}` : null, 
    fetcher
  );
  const { data: matches, mutate: mutateMatches, isLoading: loadingMatches } = useSWR(
    activeEdicaoId ? `/partidas/edicao/${activeEdicaoId}` : null, 
    fetcher
  );
  ...
  const handleAddEquipe = async (data: any) => {
    ...
    try {
      await api.post('/equipes/', {
        nome: data.nome,
        id_edicao: activeEdicaoId
      });
      mutate(`/equipes/edicao/${activeEdicaoId}`);
  ...
  const handleGerarConfrontos = async () => {
    if (!activeEdicaoId || !selectedModalidadeId) return;
    setIsGenerating(true);
    try {
      await api.post(`/edicoes/${activeEdicaoId}/gerar-confrontos`, {
  ...
  const handleAddEdicao = async (data: any) => {
    try {
      const response = await api.post('/edicoes/', {
        id_evento: parseInt(id),
        edic_ano: parseInt(data.edic_ano),
        tipo_competicao: data.tipo_competicao,
        fase_inicial: data.tipo_competicao === 'Mata-Mata' ? data.fase_inicial : null,
        data_inicio: data.data_inicio,
        data_fim: data.data_fim
      });
      mutateEdicoes();
      setEdicaoModalState({ isOpen: false, mode: 'create' });
      // Auto-selecionar a nova edição criada
      setActiveEdicaoId(response.data.id_edicao);
      alert(`Edição ${data.edic_ano} criada com sucesso!`);
    } catch (error) {
      alert('Erro ao criar edição.');
    }
  };

  const handleEditEdicao = async (data: any) => {
    if (!activeEdicaoId) return;
    try {
      await api.put(`/edicoes/${activeEdicaoId}`, {
        id_evento: parseInt(id),
        edic_ano: parseInt(data.edic_ano),
        tipo_competicao: data.tipo_competicao,
        fase_inicial: data.tipo_competicao === 'Mata-Mata' ? data.fase_inicial : null,
      ...
      mutateEdicoes();
      setEdicaoModalState({ isOpen: false, mode: 'create' });
      alert(`Edição ${data.edic_ano} atualizada com sucesso!`);
    } catch (error) {
      alert('Erro ao atualizar edição.');
    }
  };

  const handleDeleteEdicao = async (edicaoId: number) => {
    confirm({
      title: 'Excluir Edição',
      message: `Tem certeza que deseja excluir a edição ${activeEdicao?.edic_ano}? Todas as equipes, partidas e estatísticas vinculadas a esta temporada serão permanentemente deletadas.`,
      confirmText: 'Excluir',
      onConfirm: async () => {
        try {
          await api.delete(`/edicoes/${edicaoId}`);
          mutateEdicoes();
          // Reset active edition selection to another if exists, or null
          const remaining = edicoes.filter((e: any) => e.id_edicao !== edicaoId);
          if (remaining.length > 0) {
            setActiveEdicaoId(remaining[0].id_edicao);
          } else {
            setActiveEdicaoId(null);
          }
  ...
  const handleCloneTeams = async (sourceId: number) => {
    if (!activeEdicaoId) return;
    try {
      await api.post(`/edicoes/${activeEdicaoId}/clonar-equipes?edicao_origem_id=${sourceId}`);
      mutate(`/equipes/edicao/${activeEdicaoId}`);
  ...
  if (isLoadingEvento || isLoadingEdicoes) {
    return <div className="flex justify-center p-20"><Loader2 className="animate-spin text-amber-500" size={48}/></div>;
  }
  ...
  return (
    <div className="space-y-8">
      {/* Cabeçalho do Evento com Seletor de Edição */}
      <div className="bg-white rounded-3xl border border-slate-100 p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">{evento?.descricao || 'Sem descrição adicional.'}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {edicoes.length > 0 ? (
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5">
              <span className="text-xs font-semibold text-slate-500">Temporada:</span>
              <select
          ...
          {activeEdicao && (
            <>
              <button
                onClick={() => setEdicaoModalState({ isOpen: true, mode: 'edit', data: activeEdicao })}
                className="px-3 py-1.5 bg-white border border-slate-200 hover:border-slate-350 hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-xl transition-all flex items-center gap-1.5"
                title="Configurar/Editar dados desta edição"
              >
              ...
              <button
                onClick={() => handleDeleteEdicao(activeEdicao.id_edicao)}
                className="px-3 py-1.5 bg-white border border-red-200 hover:border-red-350 hover:bg-red-50/10 text-red-600 text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
                title="Excluir esta edição permanentemente"
              >
                <Trash2 size={14} />
                Excluir Edição
              </button>
          ...
          {activeEdicaoId && edicoes.length > 1 && (
            <button 
          ...
          <button
            onClick={() => setEdicaoModalState({ isOpen: true, mode: 'create' })}
            className="bg-amber-500 hover:bg-amber-600 text-white font-bold px-4 py-2 rounded-xl text-xs transition-all flex items-center gap-1.5"
          >
            <Plus size={14} />
            Nova Edição
          </button>
      ...
      {activeEdicao ? (
        <div className="space-y-6 animate-in fade-in duration-200">
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/50 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-slate-500">
            <span>Formato: <strong className="text-slate-800">{activeEdicao.tipo_competicao}</strong>{activeEdicao.tipo_competicao === 'Mata-Mata' && ` (Começa em: ${activeEdicao.fase_inicial})`}</span>
            <span className="text-slate-300">|</span>
            <span>Período: <strong className="text-slate-800">{new Date(activeEdicao.data_inicio).toLocaleDateString()} a {new Date(activeEdicao.data_fim).toLocaleDateString()}</strong></span>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  >
                    {evento?.modalidades?.map((mod: any) => (
          ...
          {/* Estado Vazio de Equipes */}
          {activeTab === 'equipes' && equipes?.length === 0 && (
            <div className="py-20 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
              <Users size={48} className="mx-auto text-slate-300 mb-4" />
              <p className="text-slate-500 font-medium">Nenhuma equipe inscrita nesta edição.</p>
              <button 
                onClick={() => setIsEquipeModalOpen(true)}
                className="mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-xl text-xs transition-all shadow-md"
              >
                Cadastrar Equipe
              </button>
            </div>
        ...
        <div className="py-20 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
          <Calendar size={48} className="mx-auto text-slate-300 mb-4" />
          <p className="text-slate-500 font-medium">Este evento não possui edições cadastradas.</p>
          <button 
            onClick={() => setEdicaoModalState({ isOpen: true, mode: 'create' })}
            className="mt-4 bg-amber-500 hover:bg-amber-600 text-white font-bold px-6 py-2.5 rounded-xl text-xs transition-all shadow-md"
          >
            Criar 1ª Edição Agora
          </button>
        </div>
      )}

      {/* Modal Inscrever Equipe */}
      {isEquipeModalOpen && activeEdicaoId && (
      ...
      {isPartidaModalOpen && activeEdicaoId && (
        <ModalAgendarPartida 
          idEdicao={activeEdicaoId.toString()}
      ...
      {/* Modal Nova/Editar Edição */}
      {edicaoModalState.isOpen && (
        <ModalEdicao 
          onSubmit={edicaoModalState.mode === 'create' ? handleAddEdicao : handleEditEdicao} 
          onCancel={() => setEdicaoModalState({ isOpen: false, mode: 'create' })} 
          initialData={edicaoModalState.data}
        />
      )}

      {/* Modal Clonar Equipes */}
      {isCloneModalOpen && activeEdicaoId && (
        <ModalClone 
          edicoesDisponiveis={edicoes.filter((e: any) => e.id_edicao !== activeEdicaoId)}
...
function MatchCard({ match, onLancarPlacar, onManageSumula, onManageStats, mutateMatches }: any) {
  ...
  const handleDelete = async () => {
    confirm({
      ...
      message: 'Deseja excluir esta partida permanentemente?',
      confirmText: 'Excluir',
      onConfirm: async () => {
        try {
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-8 my-8 animate-in zoom-in-95 border border-slate-100 relative max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            {isLocalInline ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 animate-in fade-in">
                <div className="space-y-1">
          ...
          <div className="space-y-3">
            ...
            {isArbitroInline ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 animate-in fade-in">
                <div className="space-y-1">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button type="submit" disabled={isSubmitting} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-100 text-sm">Agendar Jogo</button>
          </div>
        </form>
      </div>
    </div>
...
function ModalEstatisticas({ match, onClose }: any) {
  ...
  const handleDeleteStat = async (idStat: number) => {
    confirm({
      ...
      message: 'Remover registro de estatística?',
      confirmText: 'Remover',
      onConfirm: async () => {
        try {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <form onSubmit={handleSubmit(handleAddStat)} className="space-y-3">
              ...
              <button type="submit" className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs shadow-md shadow-blue-50 mt-4">
                Registrar Evento
              </button>
            </form>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
              {stats?.map((st: any) => {
                ...
                return (
                  <div key={st.id_estatistica} className="bg-slate-50 border border-slate-100 p-3 rounded-xl flex items-center justify-between gap-2 text-xs animate-in fade-in">
                    ...
                    <button onClick={() => handleDeleteStat(st.id_estatistica)} className="text-slate-300 hover:text-red-500 p-1">
                      <Trash2 size={14} />
                    </button>
                  </div>
                );
              ...
              {stats?.length === 0 && (
                <p className="text-center text-slate-400 italic text-xs py-8">Nenhuma ação cadastrada.</p>
              )}
            </div>
          </div>
        </div>
...
// Modal Nova/Editar Edição
function ModalEdicao({ onSubmit, onCancel, initialData }: any) {
  const { register, handleSubmit, watch, formState: { isSubmitting } } = useForm({
    defaultValues: initialData ? {
      edic_ano: initialData.edic_ano,
      tipo_competicao: initialData.tipo_competicao,
      fase_inicial: initialData.fase_inicial || 'Quartas',
      data_inicio: initialData.data_inicio ? initialData.data_inicio.substring(0, 10) : '',
      data_fim: initialData.data_fim ? initialData.data_fim.substring(0, 10) : ''
    } : {
      edic_ano: new Date().getFullYear(),
  ...
  const tipoCompeticao = watch('tipo_competicao');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-in zoom-in-95 border border-slate-100">
        <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-2">
          {isEditing ? <SlidersHorizontal className="text-amber-500" /> : <PlusCircle className="text-amber-500" />}
          {isEditing ? 'Configurar Edição' : 'Nova Edição'}
        </h2>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            <label className="text-sm font-semibold text-slate-700">Tipo de Competição</label>
            <select {...register('tipo_competicao', { required: true })} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-amber-500/20">
              <option value="Pontos Corridos">Pontos Corridos</option>
              <option value="Mata-Mata">Mata-Mata</option>
              <option value="Grupos">Grupos</option>
            </select>
          </div>
          {tipoCompeticao === 'Mata-Mata' && (
            <div className="space-y-1 animate-in slide-in-from-top-2 duration-200">
              <label className="text-sm font-semibold text-slate-700">Fase Inicial</label>
              <select {...register('fase_inicial', { required: true })} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-amber-500/20">
                <option value="Oitavas">Oitavas (16 Equipes)</option>
                <option value="Quartas">Quartas (8 Equipes)</option>
                <option value="Semifinal">Semifinal (4 Equipes)</option>
                <option value="Final">Final (2 Equipes)</option>
              </select>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-6">
            ...
            <button type="submit" disabled={isSubmitting} className="flex-1 py-3 bg-amber-500 text-white font-bold rounded-xl shadow-lg shadow-amber-100">
