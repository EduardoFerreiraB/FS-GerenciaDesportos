...
export default function EventosPage() {
  ...
  const handleDeleteEvento = async (id: number) => {
    if (confirm('Tem certeza que deseja excluir este evento? Todas as edições e partidas vinculadas podem ser afetadas.')) {
      try {
        await api.delete(`/eventos/${id}`);
        mutate('/eventos/');
      } catch (error) {
        alert('Erro ao excluir evento. Verifique se existem edições vinculadas.');
      }
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-in zoom-in-95 border border-slate-100 relative">
            ...
            <div className="mb-6">
              ...
              <p className="text-slate-500 text-sm">Defina o nome e as modalidades do evento.</p>
            </div>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="space-y-1.5">
        ...
        <input 
          ...
          placeholder="Ex: Campeonato Municipal de Futsal"
        />
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <textarea 
          {...register('descricao')}
          className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all min-h-[80px]"
      ...
      <div className="space-y-1.5">
        ...
        <select 
          ...
          className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all min-h-[120px]"
        >
      ...
      <div className="flex gap-3 pt-4">
        <button type="button" onClick={onCancel} className="flex-1 py-3 rounded-xl border border-slate-200 text-slate-600 font-semibold hover:bg-slate-50">Cancelar</button>
        <button type="submit" disabled={isSubmitting} className="flex-1 py-3 rounded-xl bg-amber-500 text-white font-bold hover:bg-amber-600 shadow-lg shadow-amber-100 disabled:opacity-70">
