...
def listar_professor(id_professor: int, db: Session = Depends(get_db)):
...
def atualizar_professor(id_professor: int, professor_atualizado: schemas.ProfessorCreate, db: Session = Depends(get_db)):
...
def excluir_professor(id_professor: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Não é possivel excluir o professor, verifique se ele tem turmas ativas.")
