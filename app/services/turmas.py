...
def criar_turma(db:Session, turma: schemas.TurmaCreate):
    ...
    for dia in turma.dias_semana:
        dia_val = dia.value if hasattr(dia, 'value') else str(dia)
        db_dia = models.TurmaDia(id_turma=db_turma.id_turma, dia_semana=dia_val)
        db.add(db_dia)
...
def atualizar_turma(db: Session, id_turma: int, turma_atualizada: schemas.TurmaUpdate):
    ...
    dias_semana_input = dados_atualizados.pop('dias_semana', None)

    for chave, valor in dados_atualizados.items():
        setattr(db_turma, chave, valor)

    if dias_semana_input is not None:
        db_turma.dias.clear()
        for dia in dias_semana_input:
            dia_val = dia.value if hasattr(dia, 'value') else str(dia)
            db_dia = models.TurmaDia(id_turma=id_turma, dia_semana=dia_val)
            db_turma.dias.append(db_dia)
